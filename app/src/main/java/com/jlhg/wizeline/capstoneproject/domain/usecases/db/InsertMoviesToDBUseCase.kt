...
class InsertMoviesToDBUseCase @Inject constructor(private val databaseRepository: com.jlhg.wizeline.local.db.DatabaseRepository) {
    ...
    suspend fun insertTopRatedMovies(movies: MutableList<MovieItem>){
        databaseRepository.insertTopRatedMovies(movies.map { it.toTopRatedMoviesEntity() })
    }

    suspend fun insertLastestMovies(movies: MutableList<MovieItem>){
        databaseRepository.insertLastestMovies(movies.map { it.toLastestMoviesEntity() })
    }
}